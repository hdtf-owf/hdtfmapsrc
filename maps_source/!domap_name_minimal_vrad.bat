...
ECHO.
title %MAPNAME% - VBSP
"%GAME_DIR%\bin\vbsp.exe" -nodefaultcubemap -game "%VPROJECT%" %MAPNAME%.vmf
color 05
ECHO.
ECHO.
ECHO.
...
ECHO.
title %MAPNAME% - VRAD
rem "%GAME_DIR%\bin\vrad.exe" -threads %CORE% -ldr -game "%VPROJECT%" %MAPNAME%.bsp
"%GAME_DIR%\bin\vrad.exe" -threads %CORE% -hdr -game "%VPROJECT%" %MAPNAME%.bsp
color 02
del /f *.prt
del /f *.vmx
del /f *.ini
